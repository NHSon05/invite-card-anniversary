    ...
    <div className="flex flex-col items-center justify-center w-full max-w-lg md:max-w-xl lg:max-w-2xl select-none">
      ...
      <div
        ...
        className={`relative w-full bg-bg-light border border-secondary rounded-2xl shadow-md transition-all duration-700 cursor-pointer overflow-hidden
          ${isOpen ? 'p-8 md:p-12 h-auto max-h-[700px] scale-100 opacity-100' : 'p-6 h-[120px] max-w-[360px] flex items-center justify-center hover:scale-105 hover:shadow-lg'}
        `}
      >
        {!isOpen ? (
          ...
          <div className="flex flex-col animate-fade-in">
            ...
            {/* Content */}
            <div className="font-['Montserrat'] font-light text-base md:text-sm text-accent-text leading-loose text-justify mb-8 whitespace-pre-line">
              {`Gửi Ty iu,
            ...
            {/* Signature */}
            <div className="text-right font-['Dancing_Script'] text-2xl text-primary border-t border-secondary pt-4">
              Hồng Sơn
            </div>
          </div>
        )}
      ...
      {isOpen && (
        <button
          onClick={onNext}
          className="mt-8 px-8 py-3 bg-primary text-white rounded-full text-sm font-medium hover:opacity-90 transition-all duration-300 shadow-sm cursor-pointer hover:shadow animate-fade-in"
        >
          Tiếp theo →
        </button>
      )}
